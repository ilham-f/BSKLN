...
var JQVMap = function (a) {
    ...
    // AJAX call to get color information based on direktorats
    $.ajax({
        url: '/getDirektoratColors',
        method: 'GET',
        dataType: 'json',
        success: function (data) {
            console.log(data); // { "ID": "#b4feb5", "DE": "#ff0000"}

            var colorData = data;

            for (var e in d.paths) {
                var f = c.canvas.createPath({ path: d.paths[e].path });
                var countryCode = e.toUpperCase(); // Ensure matching case if necessary
                var color = colorData[countryCode] || c.color; // Default to base color if not found
                ...
                jQuery(c.rootGroup).append(f);
            }

            c.canvas.canvas.appendChild(c.rootGroup);
            c.applyTransform();
            c.setColors(a.colors);
        },
                        ...
                        ? (jQuery(a.container).trigger(h, [f, d.paths[f].name]),
                          ...
                          a.showTooltip &&
                              $.ajax({
                                  ...
                                  dataType: "json",
                                  success: function (data) {
                                      // Update tooltip with dynamic data
                                      c.label.html(`
                                          <img src="${data.flag}" style="width: 20px; height: 15px; margin-right: 5px;" />
                                          <strong>${data.nama_negara}</strong><br/>
                                          Direktorat: ${data.direktorats.nama_direktorat}<br/>
                                          Kawasan: ${data.kawasans.nama_kawasan}
                                      `);
